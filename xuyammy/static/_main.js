...
var data = {
	...
	parse: function(resp){
		data.timestamp = parseFloat(resp.timestamp);
		var order = ['tags', 'stories', 'tasks'];
		for(var i = 0; i < order.length; i++)
		{
			var id = order[i];
			if(resp[id] && data['parse_' + id]) data['parse_' + id](resp[id]);
		}
		data.refresh();
